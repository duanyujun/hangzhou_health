 ...
 ***/
+(NSDictionary*)SoapMessageResultXml:(id)data ServiceMethodName:(NSString*)methodName{
    NSError *error=nil;
    ...
    while ([result count]>0) {
        ...
        if ([nodeName isEqualToString: searchStr]) {
            ...
            NSLog(@"result%@",[result[0] name]);

            NSLog(@"MsgResult%@",MsgResult);

            [resultDic setObject:MsgResult forKey:nodeName];
            MsgResult=@"";
            break;
        }
...
+(GDataXMLElement*)getBodyWithMethodName:(NSString*)methodName andGDataXMLDocument:(GDataXMLDocument*)document
{
    GDataXMLElement* rootNode = [document rootElement];
    NSString *searchStr=[NSString stringWithFormat:@"%@Result",methodName];
    NSString *searchResponse=[NSString stringWithFormat:@"%@Response",methodName];
    NSArray *resultAll=[rootNode children];
    GDataXMLElement*soapEven=[resultAll objectAtIndex:0];//<soap:Envelope xmlns:xsi=
    GDataXMLElement*soapBody=[[soapEven elementsForName:searchResponse ]objectAtIndex:0];//<soap:Body>
    GDataXMLElement*response=[[soapBody elementsForName:searchStr]objectAtIndex:0];// <GetCitiesRespons
    return response;
}
+(NSDictionary*)CheckLoginInfoWithDate:(NSData*)data andMethodName:(NSString*)methodName{

    

    NSError *error=nil;
    GDataXMLDocument *document=[[GDataXMLDocument alloc] initWithXMLString:[[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding] options:0 error:&error];
    if (error) {
        [document release];
        return nil;
    }
    ...
    NSString *MsgResult=@"";
    NSArray *result=[rootNode children];
    GDataXMLNode*getGDNO=nil;
    while ([result count]>0) {
        NSString *nodeName=[[result objectAtIndex:0] name];
        if ([nodeName isEqualToString: searchStr]) {
            getGDNO = [result objectAtIndex:0];
            MsgResult=[[result objectAtIndex:0] stringValue];
            break;
        }
        result=[[result objectAtIndex:0] children];
    }
    NSLog(@"%@",[[getGDNO children][0] name]);
