...
#define GXMLNODE_DEFINE_GLOBALS 1
#import "GXMLNode.h"

@class NSArray, NSDictionary, NSError, NSString, NSURL;
@class GXMLElement, GXMLDocument;


static const int kGXMLParseOptions = (XML_PARSE_NOCDATA | XML_PARSE_NOBLANKS);
...
// Static copy of AreEqualOrBothNil from GObject.m, so that using
// GXMLNode does not require pulling in all of G.
...
static xmlChar* GGetXMLString(NSString *str) {
...
static NSString *GFakeQNameForURIAndName(NSString *theURI, NSString *name) {

  NSString *localName = [GXMLNode localNameForName:name];
...
@interface GXMLNode (PrivateMethods)
...
- (GXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode;
...
+ (GXMLElement *)elementWithName:(NSString *)name {
...
+ (GXMLElement *)elementWithName:(NSString *)name stringValue:(NSString *)value {

  xmlNodePtr theNewNode = xmlNewNode(NULL, // namespace
                                     GGetXMLString(name));
  if (theNewNode) {

    xmlNodePtr textNode = xmlNewText(GGetXMLString(value));
    if (textNode) {
...
+ (GXMLElement *)elementWithName:(NSString *)name URI:(NSString *)theURI {
  ...
  NSString *fakeQName = GFakeQNameForURIAndName(theURI, name);

  xmlNodePtr theNewNode = xmlNewNode(NULL, // namespace
                                     GGetXMLString(fakeQName));
  if (theNewNode) {
      return [self nodeConsumingXMLNode:theNewNode];
  }
  return nil;
...
+ (id)attributeWithName:(NSString *)name stringValue:(NSString *)value {
  ...
  xmlChar *xmlName = GGetXMLString(name);
  xmlChar *xmlValue = GGetXMLString(value);
...
+ (id)attributeWithName:(NSString *)name URI:(NSString *)attributeURI stringValue:(NSString *)value {
  ...
  NSString *fakeQName = GFakeQNameForURIAndName(attributeURI, name);

  xmlChar *xmlName = GGetXMLString(fakeQName);
  xmlChar *xmlValue = GGetXMLString(value);
...
+ (id)textWithStringValue:(NSString *)value {
  ...
  xmlNodePtr theNewText = xmlNewText(GGetXMLString(value));
  if (theNewText) {
...
+ (id)namespaceWithName:(NSString *)name stringValue:(NSString *)value {
  ...
  xmlChar *href = GGetXMLString(value);
  xmlChar *prefix;

  if ([name length] > 0) {
    prefix = GGetXMLString(name);
  } else {
...
+ (id)nodeConsumingXMLNode:(xmlNodePtr)theXMLNode {
  ...
  if (theXMLNode->type == XML_ELEMENT_NODE) {
    theClass = [GXMLElement class];
  } else {
    theClass = [GXMLNode class];
  }
...
#if DEBUG
  NSCAssert(chars != NULL, @"GXMLNode sees an unexpected empty string");
#endif
...
- (void)setStringValue:(NSString *)str {
  if (xmlNode_ != NULL && str != nil) {
    ...
    if (xmlNode_->type == XML_NAMESPACE_DECL) {
      ...
      nsNode->href = xmlStrdup(GGetXMLString(str));
      ...
      xmlNodeSetContent(xmlNode_, GGetXMLString(str));
    }
  }
}
...
- (GXMLNode *)parentNode{
    if (xmlNode_ != NULL && xmlNode_->parent != NULL) {
        return [GXMLNode nodeBorrowingXMLNode:xmlNode_->parent];
    }
    return nil;
}
...
- (NSArray *)children {
  ...
  if (xmlNode_ != NULL) {
    ...
    while (currChild != NULL) {
      GXMLNode *node = [GXMLNode nodeBorrowingXMLNode:currChild];
...
- (GXMLNode *)childAtIndex:(unsigned)index {
...
- (GXMLNodeKind)kind {
  if (xmlNode_ != NULL) {
    ...
    switch (nodeType) {
      case XML_ELEMENT_NODE:         return GXMLElementKind;
      case XML_ATTRIBUTE_NODE:       return GXMLAttributeKind;
      case XML_TEXT_NODE:            return GXMLTextKind;
      case XML_CDATA_SECTION_NODE:   return GXMLTextKind;
      case XML_ENTITY_REF_NODE:      return GXMLEntityDeclarationKind;
      case XML_ENTITY_NODE:          return GXMLEntityDeclarationKind;
      case XML_PI_NODE:              return GXMLProcessingInstructionKind;
      case XML_COMMENT_NODE:         return GXMLCommentKind;
      case XML_DOCUMENT_NODE:        return GXMLDocumentKind;
      case XML_DOCUMENT_TYPE_NODE:   return GXMLDocumentKind;
      case XML_DOCUMENT_FRAG_NODE:   return GXMLDocumentKind;
      case XML_NOTATION_NODE:        return GXMLNotationDeclarationKind;
      case XML_HTML_DOCUMENT_NODE:   return GXMLDocumentKind;
      case XML_DTD_NODE:             return GXMLDTDKind;
      case XML_ELEMENT_DECL:         return GXMLElementDeclarationKind;
      case XML_ATTRIBUTE_DECL:       return GXMLAttributeDeclarationKind;
      case XML_ENTITY_DECL:          return GXMLEntityDeclarationKind;
      case XML_NAMESPACE_DECL:       return GXMLNamespaceKind;
      case XML_XINCLUDE_START:       return GXMLProcessingInstructionKind;
      case XML_XINCLUDE_END:         return GXMLProcessingInstructionKind;
      case XML_DOCB_DOCUMENT_NODE:   return GXMLDocumentKind;
    }
  }
  return GXMLInvalidKind;
}
...
- (NSArray *)nodesForXPath:(NSString *)xpath
  ...
  // are created from GXMLElement's initWithXMLString there may not be
  // a document. (We may later decide that we want to stuff the doc used
  // there into a GXMLDocument and retain it, but we don't do that now.)
  //
    ...
    if (xpathCtx) {
      ...
      if (namespaces) {
        ...
        for (NSString *prefix in namespaces) {
          ...
          if (result != 0) {
#if DEBUG
            NSCAssert1(result == 0, @"GXMLNode XPath namespace %@ issue",
                      prefix);
#endif
          }
        }
        ...
        if (nsNodePtr != NULL) {
          for (xmlNsPtr nsPtr = nsNodePtr->ns; nsPtr != NULL; nsPtr = nsPtr->next) {
            ...
            if (prefix == NULL) {
              prefix = (xmlChar*) kGXMLXPathDefaultNamespacePrefix;
            }

            int result = xmlXPathRegisterNs(xpathCtx, prefix, nsPtr->href);
            if (result != 0) {
#if DEBUG
              NSCAssert1(result == 0, @"GXMLNode XPath namespace %s issue",
                        prefix);
#endif
            }
          }
      ...
      xpathObj = xmlXPathEval(GGetXMLString(xpath), xpathCtx);
      if (xpathObj) {
        ...
        if (nodeSet) {
          ...
          for (int index = 0; index < nodeSet->nodeNr; index++) {
            ...
            GXMLNode *node = [GXMLNode nodeBorrowingXMLNode:currNode];
            if (node) {
              [array addObject:node];
            }
          }
  ...
  if (array == nil && error != nil) {
    *error = [NSError errorWithDomain:@"com.google.GXML"
                                 code:errorCode
                             userInfo:errorInfo];
  }
...
- (BOOL)isEqual:(GXMLNode *)other {
  if (self == other) return YES;
  if (![other isKindOfClass:[GXMLNode class]]) return NO;
...
- (NSUInteger)hash {
  return (NSUInteger) (void *) [GXMLNode class];
}
  ...
  if (xmlNode_ != NULL && xmlNode_->nsDef != NULL) {
    ...
    while (currNS != NULL) {
      ...
      if (!xmlStrEqual(currNS->prefix, (const xmlChar *) "xml")) {
        GXMLNode *node = [GXMLNode nodeBorrowingXMLNode:(xmlNodePtr) currNS];
...
- (void)addNamespace:(GXMLNode *)aNamespace {
...
- (void)addChild:(GXMLNode *)child {
  if ([child kind] == GXMLAttributeKind) {
...
- (void)removeChild:(GXMLNode *)child {
...
- (NSArray *)elementsForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    if (prefix) {
      ...
      xmlChar* desiredPrefix = GGetXMLString(prefix);
    ...
    for (GXMLNode *child in children) {
...
- (NSArray *)elementsForLocalName:(NSString *)localName URI:(NSString *)URI {
  ...
  if (xmlNode_ != NULL && xmlNode_->children != NULL) {
    ...
    xmlChar* desiredNSHref = GGetXMLString(URI);
    xmlChar* requestedLocalName = GGetXMLString(localName);
    ...
    if (foundParentNS == NULL) {
      NSString *fakeQName = GFakeQNameForURIAndName(URI, localName);
      expectedLocalName = GGetXMLString(fakeQName);
    }
        ...
        if (currChildPtr->nsDef != NULL) {
          ...
          if (childLocalNS != foundParentNS) {
            ...
            if (childLocalNS == NULL) {
              // no namespace found
              NSString *fakeQName = GFakeQNameForURIAndName(URI, localName);
              childDesiredLocalName = GGetXMLString(fakeQName);
            } else {
...
- (NSArray *)attributes {
  ...
  if (xmlNode_ != NULL && xmlNode_->properties != NULL) {
    ...
    while (prop != NULL) {
      ...
      GXMLNode *node = [GXMLNode nodeBorrowingXMLNode:(xmlNodePtr) prop];
      if (array == nil) {
        array = [NSMutableArray arrayWithObject:node];
      } else {
        [array addObject:node];
...
- (void)addAttribute:(GXMLNode *)attribute {
...
- (GXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode {
  // search the cached attributes list for the GXMLNode with
  // the underlying xmlAttrPtr
  NSArray *attributes = [self attributes];

  for (GXMLNode *attr in attributes) {
...
- (GXMLNode *)attributeForName:(NSString *)name {

  if (xmlNode_ != NULL) {

    xmlAttrPtr attrPtr = xmlHasProp(xmlNode_, GGetXMLString(name));
    if (attrPtr == NULL) {
      ...
      if (prefix) {
        ...
        ns = xmlSearchNs(xmlNode_->doc, xmlNode_, GGetXMLString(prefix));
      }

      const xmlChar* nsURI = ((ns != NULL) ? ns->href : NULL);
      attrPtr = xmlHasNsProp(xmlNode_, GGetXMLString(name), nsURI);
    }

    if (attrPtr) {
      GXMLNode *attr = [self attributeForXMLNode:attrPtr];
      return attr;
    }
  }
  return nil;
...
- (GXMLNode *)attributeForLocalName:(NSString *)localName
                                    URI:(NSString *)attributeURI {

  if (xmlNode_ != NULL) {

    const xmlChar* name = GGetXMLString(localName);
    const xmlChar* nsURI = GGetXMLString(attributeURI);
    ...
    if (attrPtr == NULL) {
      ...
      NSString *fakeQName = GFakeQNameForURIAndName(attributeURI, localName);
      const xmlChar* xmlFakeQName = GGetXMLString(fakeQName);
...
- (NSString *)resolvePrefixForNamespaceURI:(NSString *)namespaceURI {
  ...
  if (xmlNode_ != NULL) {
    ...
    xmlChar* desiredNSHref = GGetXMLString(namespaceURI);
...
@interface GXMLDocument (PrivateMethods)
- (void)addStringsCacheToDoc;
@end

@implementation GXMLDocument

- (id)initWithXMLString:(NSString *)str options:(unsigned int)mask error:(NSError **)error {

  NSData *data = [str dataUsingEncoding:NSUTF8StringEncoding];
  GXMLDocument *doc = [self initWithData:data options:mask error:error];
  return doc;
}
...
- (id)initWithData:(NSData *)data options:(unsigned int)mask error:(NSError **)error {
  ...
  if (self) {
    ...
    xmlDoc_ = xmlReadMemory((const char*)[data bytes], (int)[data length], baseURL, encoding,
                            kGXMLParseOptions); // TODO(grobbins) map option values
    if (xmlDoc_ == NULL) {
      if (error) {
       *error = [NSError errorWithDomain:@"com.google.GXML"
                                    code:-1
...
- (id)initWithRootElement:(GXMLElement *)element {
...
- (void)addStringsCacheToDoc {
  ...
  NSCAssert(xmlDoc_ != NULL && xmlDoc_->_private == NULL,
            @"GXMLDocument cache creation problem");
#endif
...
- (GXMLElement *)rootElement {
  GXMLElement *element = nil;

  if (xmlDoc_ != NULL) {
    xmlNodePtr rootNode = xmlDocGetRootElement(xmlDoc_);
    if (rootNode) {
      element = [GXMLElement nodeBorrowingXMLNode:rootNode];
    }
  }
  return element;
}
...
- (void)setVersion:(NSString *)version {
  ...
  if (xmlDoc_ != NULL) {
    ...
    if (version != nil) {
      xmlDoc_->version = xmlStrdup(GGetXMLString(version));
    }
  }
}
...
- (void)setCharacterEncoding:(NSString *)encoding {
  ...
  if (xmlDoc_ != NULL) {
    ...
    if (encoding != nil) {
      xmlDoc_->encoding = xmlStrdup(GGetXMLString(encoding));
    }
  }
}
...
- (NSArray *)nodesForXPath:(NSString *)xpath
                namespaces:(NSDictionary *)namespaces
                     error:(NSError **)error {
  if (xmlDoc_ != NULL) {
    GXMLNode *docNode = [GXMLElement nodeBorrowingXMLNode:(xmlNodePtr)xmlDoc_];
