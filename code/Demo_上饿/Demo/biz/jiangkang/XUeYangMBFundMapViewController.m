...
@interface XUeYangMBFundMapViewController ()

@end

@implementation XUeYangMBFundMapViewController
...
- (void)viewDidLoad
{
    ...
    for (int i=0; i<reseDataArray.count; i++) {
        [one addObject:MBNonEmptyStringNo_(reseDataArray[i][@"HD46"])];
        [two addObject:MBNonEmptyStringNo_(reseDataArray[i][@"HD47"])];
    ...
    for (int i=0; i<2; i++) {
        ...
        UILabel *label =[[UILabel alloc]initWithFrame:CGRectMake(50+i*(140),  kContentViewHeight-75, 150, 20)];
        if (i==0) {
