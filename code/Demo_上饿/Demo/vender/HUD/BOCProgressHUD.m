...
- (void)updateProgress {
    [(BOCRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeDeterminate) {
        self.indicator = [[BOCRoundProgressView alloc] init];
    }
...
+ (void)showHUDViewTo:(UIView *)view
    ...
    BOCProgressHUD *hud = [[BOCProgressHUD alloc] initWithView:view];
...
+ (BOCProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    [BOCProgressHUD hideHUDForView:view animated:animated];
	BOCProgressHUD *hud = [[BOCProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
	return hud;
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[BOCProgressHUD class]]) {
            BOCProgressHUD *HUD = (BOCProgressHUD *)v;
            HUD.removeFromSuperViewOnHide = YES;
            [HUD hide:animated];
		}
	}
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        CGSize dims = 
		[self.labelText sizeWithFont:self.labelFont constrainedToSize:CGSizeMake(200, 50) lineBreakMode:NSLineBreakByCharWrapping];
...
@implementation BOCRoundProgressView
