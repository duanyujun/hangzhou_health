...
+(NSString*)methodSendLogSoapMessage:(NSString*)methodName{
    NSMutableString *soap=[NSMutableString stringWithFormat:@"<%@ xmlns=\"%@\"><po>",methodName,defaultWebServiceNameSpace];
    [soap appendString:@"%@"];
    [soap appendFormat:@"</po></%@>",methodName];
    return [NSString stringWithFormat:[self defaultSendLogSoapMesage],soap];
}
...
+(NSString*)arraySendLogToDefaultSoapMessage:(NSArray*)arr methodName:(NSString*)methodName{
    if ([arr count]==0||arr==nil) {
        return [NSString stringWithFormat:[self methodSendLogSoapMessage:methodName],@""];
    }
    ...
    return [NSString stringWithFormat:[self methodSendLogSoapMessage:methodName],msg];
}
