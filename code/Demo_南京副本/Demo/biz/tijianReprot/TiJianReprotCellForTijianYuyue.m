...
@implementation TiJianReprotCellForTijianYuyue
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _itemNameLbl =[[MBLabel alloc]initWithFrame:CGRectMake(40, 10, 290, 30)];
        _itemNameLbl.font =[UIFont fontWithName:@"Helvetica Neue" size:14];
        _itemNameLbl.backgroundColor=[UIColor clearColor];
        [self addSubview:_itemNameLbl];
        
        _itemLbl =[[MBLabel alloc]initWithFrame:CGRectMake(40, 35, 230, 60)];
        ...
        seeDetailBtn.frame=CGRectMake(245, 50, self.frame.size.height+10+50, self.frame.size.height);
        [seeDetailBtn setTitle:@"详情" forState:UIControlStateNormal];
        [seeDetailBtn setTitleColor:[UIColor blackColor] forState:UIControlStateHighlighted];
        [seeDetailBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];

        [seeDetailBtn addTarget:self action:@selector(seeDetailBtnPressed) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:seeDetailBtn];
...
-(void)seeDetailBtnPressed
{
    
    if (_delegateAbout&&[_delegateAbout respondsToSelector:@selector(seeDetailBtnPressed:)]) {
        [_delegateAbout seeDetailBtnPressed:self];
    }
}
